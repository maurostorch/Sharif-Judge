...
PASSEDTESTS=0
INITIAL_TIMELIMIT = $TIMELIMIT
INITIAL_TIMELIMITINT = $TIMELIMITINT
INITIAL_MEMORY = $MEMLIMIT
for((i=1;i<=TST;i++)); do
	shj_log "\n=== TEST $i ==="
	...
	# added for handle per-testcase time and memor limit
	TIMELIMIT = $INITIAL_TIMELIMIT
	TIMELIMITINT = $INITIAL_TIMELIMITINT
	MEMLIMIT = $INITIAL_MEMORY
	if [[ -e "$PROBLEMPATH/config/time.txt" ]]
	then
		TIMELIMIT = `cat $PROBLEMPATH/config/time.txt`;
		TIMELIMITINT = "${TIMELIMIT/.*}";
	fi
	if [[ -e "$PROBLEMPATH/config/mem.txt" ]]
	then
		MEMLIMIT = `cat $PROBLEMPATH/config/mem.txt`;
	fi
